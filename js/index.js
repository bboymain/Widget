  ...
  app.plug = {
    ...
    getActions: function(){
      ...
      $(window).scroll(function(){
        ...
        if(thisTop <= 0){
          $('[data-fn="sticker"]').css({opacity:0}).animate({top:0},100,function(){
            $(this).css({position:'relative',opacity:1});
          });
        }else{
          $('[data-fn="sticker"]').css({position:'absolute'}).animate({top:thisTop,width:'100%',zIndex: 1000},100);
        }
      });
      ...
      // tabs
      $('[data-fn="tabs"]').on(app.plug.mobilecheck() ? 'touchstart' : 'click',function(e){
        e.preventDefault();
      ...
      $('[data-fn="accordion"]').on(app.plug.mobilecheck() ? 'touchstart' : 'click',function(e){
        e.preventDefault();
        ...
        if($('#'+open).hasClass('accordion-open')){
          ...
          $('#'+open).addClass('accordion-open').parent().removeClass('accordion-open');
      ...
      // modal
      $('[data-fn="text"]').on(app.plug.mobilecheck() ? 'touchstart' : 'click',function(e){
        e.preventDefault();
      ...
      $('[data-fn="img"]').on(app.plug.mobilecheck() ? 'touchstart' : 'click',function(e){
        e.preventDefault();
        $('.overlay').css('display','block');
        $('body').css('overflow','hidden');
        $('.modal').animate({
      ...
      $('.overlay,#close').on(app.plug.mobilecheck() ? 'touchstart' : 'click',function(e){
        e.preventDefault();
        $('.modal').animate({
          left:'-200%'
        },1000,function(){
